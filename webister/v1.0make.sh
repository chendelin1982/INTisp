...
if command_exists dialog ; then
dialog --title "IntISP $VERSION" --backtitle "Adaclare Technologies" --msgbox 'Welcome to the IntISP setup wizard. This installer will guide you through the installation wizard of IntISP.' 6 100
fi
...
if [ -e /usr/local/cpanel ] || [ -e /usr/local/directadmin ] || [ -e /usr/local/solusvm/www ] || [ -e /usr/local/home/admispconfig ] || [ -e /usr/local/lxlabs/kloxo ] || [ -e /var/webister ]; then
   if command_exists dialog ; then
    dialog --title "Warning" --backtitle "Adaclare Technologies" --msgbox 'It appears that a control panel is already installed on your server.' 6 20
    else
...
cmd=(dialog --backtitle "Adaclare Technologies" --keep-tite  --menu "Do you own a licence for IntISP/webister?" 22 76 16)
...
(
    ...
    while [ $processed -le $items ]; do
        ...
        echo "XXX"
        echo "Welcome to Adaclare IntISP\nThis is now installing IntISP, You are done for right now. We will tell you if we need you again.\n\n$MESSAGE"
        echo "XXX"
        echo "$pct"
...
) |  dialog --clear --title "IntISP Installer $VERSION" --backtitle "Adaclare Technologies" --keep-tite --no-ok --no-cancel --no-kill --gauge "Welcome to Adaclare IntISP\nThis is now installing IntISP, You are done for right now. We will tell you if we need you again." 10 60 0
fi
...
BLAH=$(rm -rf /var/www/html && cp -R website /var/www/html && cp -R interface /var/www/html/web) & spinner
progressbar 5 20
installer "Creating Service" 25
BLAH=$(cp files/service /etc/init.d/webister) & spinner
BLAH=$(cp files/php.ini /var/webister/) & spinner
progressbar 6 20
...
BLAH=$(chmod -R 777 /var/www/html/web/config.php) & spinner
progressbar 19 20
